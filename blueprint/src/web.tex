...
\begin{definition}
    ...
    Let $U$ be an open connected set of a Banach space $E$.
    For $a, b \in U$, we define $d_U(a,b)$ to be the infimum of the lengths of the polygonal lines contained in $U$ joining $a$ and $b$.
\end{definition}
...
\end{theorem}
\begin{proof}
    Let $\varphi \colon [0,1] \to U$  be an arbitrary polygonal line joining $a$ to $b$ in $U$,
    and $t_0,\dots,t_n \in [0,1]$ the finitely many points such that
    $\varphi \colon [t_i, t_{i+1}] \to U$ is affine for all $i \in \{0, \dots, n-1\}$.

    \textit{Positivity}:
    The length of $\varphi$ is $L((t_i), n, \varphi) := \sum_{i=0}^{n-1}||\varphi(t_{i+1}) - \varphi(t_i)||$,
    ...
    the reverse path $\psi:[0,1] \to U$ defined by $\psi(x) = \varphi(1-x)$ is a polygonal line from $b$ to $a$.
    Length($\psi$) $= \sum_{i=0}^{n-1}||\psi(1-t_i)-\psi(1-t_{i+1})|| = \sum_{i=0}^{n-1}||\varphi(t_{i+1})-\varphi(t_i)|| =$ Length($\varphi$),
    ...
    Let $\varphi:[0,1] \to U$ be a polygonal line from $a$ to $b$ and $\psi:[0,1] \to U$ be a polygonal line from $b$ to $c$.
    The extension of $\varphi$ by $\psi$ is a polygonal line $\theta:[0,1] \to U$ from $a$ to $c$, where
    \[
    \theta(x) = 
        \begin{cases}
        ...
        \end{cases}
    \]
    By the definition of infimum, $d_U(a,c) \leq$ Length($\theta$) $=$ Length($\varphi$) $+$ Length($\psi$).
    Since $\varphi$ and $\psi$ are arbitrary, $d_U(a,c) \leq d_U(a,b) + d_U(b,c)$.
\end{proof}
